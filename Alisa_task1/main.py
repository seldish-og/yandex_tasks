...
def handle_dialog(req, res):
    ...
    if req['request']['original_utterance'].lower() in [
        ...
        if sessionStorage[user_id]['stage'] == 'кролика':
            # Пользователь согласился, прощаемся.
            res['response']['text'] = 'Кролика можно найти на Яндекс.Маркете!'
            res['response']['end_session'] = True
            return
        else:
            sessionStorage[user_id]['stage'] = 'кролика'
            res['response']['text'] = \
    ...
    res['response']['text'] = \
        f"Все говорят '{req['request']['original_utterance']}', а ты купи {sessionStorage[user_id]['stage']}!"
    res['response']['buttons'] = get_suggests(user_id)
...
def get_suggests(user_id):
    ...
    if len(suggests) < 2 and sessionStorage[user_id]['stage'] == 'кролика':
        suggests.append({
            "title": "Ладно",
